...
namespace MASA_Ads_Extractor.LinksScrapers
{
	...
	{
		// Token: 0x060000CC RID: 204 RVA: 0x0000E96C File Offset: 0x0000CB6C
		public static void GetLinks(WebView WB, MainForm mainForm)
		{
			Element NextButton = null;
			for (;;)
			{
				YelpFRLinksScraper.AutoClosingMessageBox.Show("Please Wait...", "Y-Leads Extractor", 1500);
				...
				{
					YelpFRLinksScraper.WaitForBrowser(WB);
					...
					if (flag)
					{
						...
						DataItem.DetailsLink = "https://www.autoscout24.it" + WebScraper.GetParent(WB, Title[0])["href"].ToString();
					}
					else
					{
		...
		// Token: 0x060000CD RID: 205 RVA: 0x0000EEA8 File Offset: 0x0000D0A8
		private static void GetData(ref List<DataItem> DataItems, MainForm mainForm)
		{
			...
			YelpFRDataScraper[] Pool = new YelpFRDataScraper[PoolSize];
			int ScraperIndex = 0;
			int Completed = 0;
			int i = 0;
			while (i < PoolSize && i < DataItems.Count)
			{
				Pool[i] = new YelpFRDataScraper(DataItems[i], Program.AppSettings, mainForm.ProxyServers);
				ScraperIndex++;
				i++;
			}
			ScraperIndex--;
			...
			while (EmptyPoolPositions < PoolSize)
			{
				...
				for (int j = 0; j < PoolSize; j++)
				{
					...
					if (flag)
					{
						...
						if (flag2)
						{
							Pool[j] = new YelpFRDataScraper(DataItems[ScraperIndex], Program.AppSettings, mainForm.ProxyServers);
						}
						else
						{
							Pool[j] = null;
		...
		// Token: 0x060000CE RID: 206 RVA: 0x0000EFF4 File Offset: 0x0000D1F4
		private static void UpdateTable(DataItem Item, MainForm mainForm, float ProgressValue)
		{
			for (int i = 0; i < mainForm.dgvResults.Rows.Count; i++)
			{
				...
				if (flag)
				{
					mainForm.dgvResults.Rows[i].Cells[4].Value = YelpFRLinksScraper.ClearValue(Item.State);
					mainForm.dgvResults.Rows[i].Cells[5].Value = YelpFRLinksScraper.ClearValue(Item.PostalCode);
					mainForm.dgvResults.Rows[i].Cells[6].Value = YelpFRLinksScraper.ClearValue(Item.Country);
					mainForm.dgvResults.Rows[i].Cells[7].Value = YelpFRLinksScraper.ClearValue(Item.Phone);
					mainForm.dgvResults.Rows[i].Cells[10].Value = YelpFRLinksScraper.ClearValue(Item.Email);
					mainForm.dgvResults.Rows[i].Cells[11].Value = YelpFRLinksScraper.ClearValue(Item.MapLink);
		...
		// Token: 0x060000CF RID: 207 RVA: 0x0000F194 File Offset: 0x0000D394
		...
		// Token: 0x060000D0 RID: 208 RVA: 0x0000F1D0 File Offset: 0x0000D3D0
		...
		// Token: 0x060000D1 RID: 209 RVA: 0x0000F20C File Offset: 0x0000D40C
		...
		// Token: 0x060000D2 RID: 210 RVA: 0x0000F248 File Offset: 0x0000D448
		...
		{
			// Token: 0x0600019C RID: 412 RVA: 0x0001F76C File Offset: 0x0001D96C
			...
			// Token: 0x0600019D RID: 413 RVA: 0x0001F7D0 File Offset: 0x0001D9D0
			public static void Show(string text, string caption, int timeout)
			{
				new YelpFRLinksScraper.AutoClosingMessageBox(text, caption, timeout);
			}

			// Token: 0x0600019E RID: 414 RVA: 0x0001F7DC File Offset: 0x0001D9DC
			private void OnTimerElapsed(object state)
			{
				IntPtr mbWnd = YelpFRLinksScraper.AutoClosingMessageBox.FindWindow("#32770", this._caption);
				bool flag = mbWnd != IntPtr.Zero;
				if (flag)
				{
					YelpFRLinksScraper.AutoClosingMessageBox.SendMessage(mbWnd, 16U, IntPtr.Zero, IntPtr.Zero);
				}
