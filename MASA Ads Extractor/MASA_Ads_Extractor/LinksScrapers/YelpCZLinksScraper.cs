...
namespace MASA_Ads_Extractor.LinksScrapers
{
	...
	{
		// Token: 0x060000B2 RID: 178 RVA: 0x0000C3D4 File Offset: 0x0000A5D4
		public static void GetLinks(WebView WB, MainForm mainForm)
		{
			Element NextButton = null;
			for (;;)
			{
				YelpCZLinksScraper.AutoClosingMessageBox.Show("Please Wait...", "AutoScout24 Extractor", 1500);
				...
				foreach (Element HItem in HTMLItems)
				{
					...
					if (flag)
					{
						...
						DataItem.DetailsLink = "https://www.autoscout24.de" + WebScraper.GetParent(WB, Title[0])["href"].ToString();
					}
					else
					{
		...
		// Token: 0x060000B3 RID: 179 RVA: 0x0000C904 File Offset: 0x0000AB04
		...
		// Token: 0x060000B4 RID: 180 RVA: 0x0000CA50 File Offset: 0x0000AC50
		private static void UpdateTable(DataItem Item, MainForm mainForm, float ProgressValue)
		{
			for (int i = 0; i < mainForm.dgvResults.Rows.Count; i++)
			{
				...
				if (flag)
				{
					mainForm.dgvResults.Rows[i].Cells[4].Value = YelpCZLinksScraper.ClearValue(Item.State);
					mainForm.dgvResults.Rows[i].Cells[5].Value = YelpCZLinksScraper.ClearValue(Item.PostalCode);
					mainForm.dgvResults.Rows[i].Cells[6].Value = YelpCZLinksScraper.ClearValue(Item.Country);
					mainForm.dgvResults.Rows[i].Cells[7].Value = YelpCZLinksScraper.ClearValue(Item.Phone);
					mainForm.dgvResults.Rows[i].Cells[10].Value = YelpCZLinksScraper.ClearValue(Item.Email);
					mainForm.dgvResults.Rows[i].Cells[11].Value = YelpCZLinksScraper.ClearValue(Item.MapLink);
		...
		// Token: 0x060000B5 RID: 181 RVA: 0x0000CBF0 File Offset: 0x0000ADF0
		...
		// Token: 0x060000B6 RID: 182 RVA: 0x0000CC2C File Offset: 0x0000AE2C
		...
		// Token: 0x060000B7 RID: 183 RVA: 0x0000CC68 File Offset: 0x0000AE68
		...
		// Token: 0x060000B8 RID: 184 RVA: 0x0000CCA4 File Offset: 0x0000AEA4
		...
		{
			// Token: 0x0600018D RID: 397 RVA: 0x0001F52C File Offset: 0x0001D72C
			...
			// Token: 0x0600018E RID: 398 RVA: 0x0001F590 File Offset: 0x0001D790
			public static void Show(string text, string caption, int timeout)
			{
				new YelpCZLinksScraper.AutoClosingMessageBox(text, caption, timeout);
			}

			// Token: 0x0600018F RID: 399 RVA: 0x0001F59C File Offset: 0x0001D79C
